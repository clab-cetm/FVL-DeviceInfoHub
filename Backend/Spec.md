...
#### Static File Serving
HTTP server also serves static files (e.g. the Frontend console's `index.html`, `js/`, `css/`) from a configurable directory. The first segment of an incoming request URL is matched against the `databases` map first; only requests that do NOT match a known database name fall through to static file serving.

Server config include:
- static_dir : Path to the directory whose files will be served. Resolved relative to the server's working directory (i.e. where the executable is launched from). Defaults to `.` if omitted. Example: `"../Frontend"` lets the Backend executable serve the Frontend folder one level up.

The resolved absolute path is logged at startup as `[HTTP] serving static files from <path>` so you can verify the path is correct.
